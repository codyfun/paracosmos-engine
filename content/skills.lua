local skills = {
	attack = {
		...
		GetExDesc = function (self, target)
			return string.gsub("Do <color=red>{low}-{high}</color> damage to {target.name}\n\n<color=red>Attack</color> vs. <color=blue>Defense</color> = <color=red>{user.attack}</color>/<color=blue>{target.defense}</color>x{power}<color=grey>~10%</color>", "%{(.-)%}", function(s)
	...
	heal = {
		...
		GetExDesc = function (self, target)
			return string.gsub("Restore <color=green>{low}-{high}</color> HP to {target.name}\n\n<color=purple>Magic Attack</color>: <color=purple>{user.magicattack}</color>x{power}<color=grey>~10%</color>", "%{(.-)%}", function(s)
				return GL.GetExDesc_common(self, s, target)
			end), true
		end,
		OnUse = function (self, target)
	...
	buff_attack = {
		...
		target = GL.target_funcs[GL.TARGET_ALLY],
		OnUse = function (self, target)
			target:AddStatus("buffed_attack")
